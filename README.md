...
- **searxng_web_search**
  ...
  - Inputs:
    ...
    - `result_detail` (string, optional): `"full"` (the default) preserves SearXNG metadata, warnings, provenance, answers, infoboxes, corrections, and suggestions. `"compact"` returns only title, description, and URL for every result, with predictable text or JSON shapes. Use full when those research signals matter.
