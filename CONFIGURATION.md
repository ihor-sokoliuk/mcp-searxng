...
| `MCP_HTTP_EXPOSE_FULL_CONFIG` | No | `false` | Include configured `SEARXNG_URL` hosts (with URL credentials redacted) in the `config://server-config` MCP resource when hardened mode is active; otherwise the resource reports only whether a URL is configured. No effect on `/health`. |
