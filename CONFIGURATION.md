...
| `MCP_HTTP_EXPOSE_FULL_CONFIG` | No | `false` | Expose the full configuration (including configured `SEARXNG_URL` hosts with URL credentials redacted) in the `config://server-config` MCP resource when hardened mode is active. No effect on `/health`. |
