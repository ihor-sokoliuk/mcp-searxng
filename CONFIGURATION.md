...
| `MCP_HTTP_EXPOSE_FULL_CONFIG` | No | `false` | Expose the full configuration (including redacted `SEARXNG_URL` hosts) in the `config://server-config` MCP resource when hardened mode is active. No effect on `/health`. |
