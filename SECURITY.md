...
Modern HTTP clients must provide `MCP-Protocol-Version`. A narrow temporary guard in the published SDK `2.0.0` rejects only a safe 2026-07-28 request shape missing that header; it is removed only after the first stable SDK with upstream PR 2594 is proven to provide the same contract.
