...
export const LITE_READ_URL_TOOL: Tool = {
  name: "web_url_read",
  description:
    "Fetch URL. Converts HTML to markdown; returns explicit JSON, plain text, YAML, TOML, and XML as readable markdown; supports bounded PDF text extraction; other binary/media/archive downloads are rejected. When FlareSolverr is configured, mcp-searxng attempts to acquire a browser session before every uncached URL read; busy or unavailable attempts fall back to an uncached direct read.",
  inputSchema: {
    type: "object",
...
export const READ_URL_TOOL: Tool = {
  ...
  description:
    ...
    "When the operator configures FlareSolverr, mcp-searxng attempts to acquire a browser session before every uncached URL read; cache hits bypass acquisition and busy or unavailable attempts use an uncached direct-fetch fallback. " +
    "Three modes: " +
