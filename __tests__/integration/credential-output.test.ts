...
type Fixture = {
  password: string;
  auth: string;
  observed: Array<{path: string; auth: string | undefined}>;
...
function serveSolver(req: http.IncomingMessage, res: http.ServerResponse) {
  let body = '';
...
function serveSearch(url: URL, res: http.ServerResponse, fixture: Fixture) {
  res.setHeader('content-type', 'application/json');
  if (url.pathname.endsWith('/config')) {
    if (fixture.configFailure) { res.writeHead(401, `Denied ${fixture.auth}`); res.end(); }
    else res.end(JSON.stringify({plugins:[fixture.auth]}));
  } else if (url.pathname.endsWith('/autocompleter')) {
    res.end(JSON.stringify(['query',[fixture.auth]]));
  } else {
    const content = url.searchParams.get('q') === 'safe' ? 'Ordinary content' : fixture.auth;
    res.end(JSON.stringify({results:[{title:'Fixture',url:'https://example.test/',content}]}));
  }
}

function serveDocument(url: URL, res: http.ServerResponse, fixture: Fixture) {
  if (url.pathname.endsWith('/pdf')) {
    res.setHeader('content-type','application/pdf'); res.end(createTextPdf([fixture.password]));
  } else if (url.pathname.endsWith('/json')) {
    res.setHeader('content-type','application/json'); res.end(JSON.stringify({data:fixture.password}));
  } else {
    res.setHeader('content-type','text/html'); res.end('<h1>Fixture</h1><p>Before ' + fixture.password + ' after</p>');
  }
}

function createTarget(fixture: Fixture) {
  return http.createServer((req,res) => {
    const url = new URL(req.url ?? '/', 'http://localhost');
    fixture.observed.push({path:url.pathname,auth:req.headers.authorization});
    if (url.pathname === '/v1') serveSolver(req, res);
    else if (url.pathname.startsWith('/subpath/')) serveSearch(url, res, fixture);
    else serveDocument(url, res, fixture);
  });
}

function configure(targetUrl: string, password: string) {
  for (const key of Object.keys(process.env)) {
    if (/proxy|^AUTH_|^SEARXNG_|^MCP_HTTP_|^FLARESOLVERR|^BYPARR/i.test(key)) delete process.env[key];
  }
...
async function connectClient(client: Client, kind: string, onStderr: (chunk: unknown) => void) {
  if (kind === 'stdio') {
  ...
  const server = http.createServer(await createHttpServer(modern => createMcpServer(controller, modern)));
  try {
...
async function checkSearchTools(client: Client, fixture: Fixture) {
  const resource = await client.readResource({uri:'config://server-config'});
  assert.ok(!JSON.stringify(resource).includes(fixture.password));
  ...
  for (let attempt=0; attempt<2; attempt++) {
    ...
    assert.ok(!JSON.stringify(info).includes(fixture.auth));
  }
  assert.equal(fixture.observed.filter(r=>r.path.endsWith('/config')).length,1,'negative cache retained');
  fixture.configFailure=false;
  const info = await client.callTool({name:'searxng_instance_info',arguments:{refresh:true}});
  assert.equal(info.isError,true);
}

async function checkDocuments(client: Client, targetUrl: string, password: string) {
...
async function checkCacheAndSolver(client: Client, targetUrl: string, fixture: Fixture) {
  const cached = targetUrl + '/cached';
  urlCache.set(cached, `Before ${fixture.password} after`);
  const result = await client.callTool({name:'web_url_read',arguments:{url:cached,startChar:7,maxLength:5}});
  assert.equal(result.isError,true);
  assert.ok(!fixture.observed.some(r=>r.path === '/cached'));
  ...
  assert.ok(fixture.observed.some(r=>r.path === '/v1'));
  assert.ok(fixture.observed.some(r=>r.path === '/solver-page'));
}

async function runWireCase(kind: string) {
  ...
  const fixture: Fixture = {password, auth, observed:[], configFailure:true};
  ...
  try {
    configure(targetUrl, password);
    mcpHttp = await connectClient(client, kind, chunk => {stderr += String(chunk);});
    await checkSearchTools(client, fixture);
    await checkDocuments(client, targetUrl, password);
    if (kind === 'http') await checkCacheAndSolver(client, targetUrl, fixture);
    assert.ok(fixture.observed.filter(r=>r.path.startsWith('/subpath/')).every(r=>r.auth === auth));
    assert.ok(fixture.observed.filter(r=>!r.path.startsWith('/subpath/')).every(r=>r.auth === undefined));
    await client.close();
...
export async function runTests() {
  ...
  for (const kind of ['stdio','http']) {
    await testFunction(`real ${kind} content boundaries preserve wire auth and withhold reflected credentials`, () => runWireCase(kind), results);
  }
